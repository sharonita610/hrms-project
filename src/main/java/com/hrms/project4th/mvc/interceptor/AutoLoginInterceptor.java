...
import static com.hrms.project4th.mvc.util.LoginUtil.AUTO_LOGIN_CHECK;
...
public class AutoLoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        Cookie cookie = WebUtils.getCookie(request, AUTO_LOGIN_CHECK);

        if(cookie != null){
            // 2. 쿠키에 저장된 세션 아이디를 읽는다.
            String empSession = cookie.getValue();

            // 3. DB에서 세션 id로 회원 정보를 조회한다.
            Employees foundEmployee = employeesMapper.checkSession(empSession);
